# 리쿠가 옆으로 넘어가는 곳으로 긑남
# 이걸 앞으로 velocity 가 아니라 실제 간거리를 reward 로 바구는 작업중
...
class LikuEnvCfg(DirectRLEnvCfg):
    ...
    # 다리 6개용 기어
    joint_gears: list = [5000.0] * 6

    # reward
    heading_weight: float = 0.5
    up_weight: float = 0.3
    energy_cost_scale: float = 0.1
    actions_cost_scale: float = 0.1
    alive_reward_scale: float = 0.4
    dof_vel_scale: float = 0.1
    death_cost: float = -1.0
    termination_height: float = 1.85
...
class LikuEnv(LocomotionEnv):
    ...
    def _debug_current_axis_vectors(self):
        env_id = self._debug_env_id

        q = self._get_root_quat_w()[env_id:env_id + 1]

        up_local = self._robot_up_axis_local
        fwd_local = self._robot_forward_axis_local

        up_world = quat_rotate_wxyz(q, up_local)
        fwd_world = quat_rotate_wxyz(q, fwd_local)

        up_world = F.normalize(up_world, dim=-1)
        fwd_world = F.normalize(fwd_world, dim=-1)

        world_up = F.normalize(self._world_up_axis, dim=-1)
        world_fwd = F.normalize(self._world_forward_axis, dim=-1)

        up_proj = torch.sum(up_world * world_up, dim=-1)
        heading_proj = torch.sum(fwd_world * world_fwd, dim=-1)

        logger.info("==================================================")
        logger.info("[INITIAL_AXIS]")
        logger.info(f"root_quat_w = {q[0].detach().cpu().tolist()}")

        logger.info(
            f"robot HEAD/UP local axis = {up_local[0].detach().cpu().tolist()}"
        )
        logger.info(
            f"robot HEAD/UP world dir  = {[round(x, 4) for x in up_world[0].detach().cpu().tolist()]}"
        )
        logger.info(
            f"target world up axis     = {world_up[0].detach().cpu().tolist()}"
        )
        logger.info(
            f"up_proj = {up_proj[0].item():.4f}  "
            f"(+1이면 머리가 월드 +Z를 정확히 봄)"
        )

        logger.info(
            f"robot FORWARD local axis = {fwd_local[0].detach().cpu().tolist()}"
        )
        logger.info(
            f"robot FORWARD world dir  = {[round(x, 4) for x in fwd_world[0].detach().cpu().tolist()]}"
        )
        logger.info(
            f"target world forward     = {world_fwd[0].detach().cpu().tolist()}"
        )
        logger.info(
            f"heading_proj = {heading_proj[0].item():.4f}  "
            f"(+1이면 로봇 전진축이 월드 +X를 정확히 봄)"
        )
        logger.info("==================================================")
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # --------------------------------------------------
        # Forward velocity reward 추가
        # 로봇의 local forward axis가 월드에서 어느 방향인지 구하고,
        # 실제 root linear velocity가 그 방향으로 얼마나 나오는지 계산
        # --------------------------------------------------
        root_quat = self._get_root_quat_w()
        ...
        forward_vel = torch.sum(root_lin_vel_w * fwd_world, dim=-1)

        target_forward_vel = 0.8

        forward_vel_clipped = torch.clamp(forward_vel, min=0.0, max=target_forward_vel)
        ...
        joint_limit_penalty = torch.sum(
            torch.clamp(torch.abs(q_all) - 1.2, min=0.0) ** 2,
            dim=-1,
        )
        ...
        posture_gate = torch.where(
            custom_up_proj > 0.97,
            torch.ones_like(custom_up_proj),
            torch.zeros_like(custom_up_proj),
        )
        ...
        forward_reward = (
            self.cfg.forward_reward_scale
            * forward_vel_clipped
            / target_forward_vel
        )
        forward_reward = forward_reward * posture_gate * joint_gate
        ...
        if self.common_step_counter % self._debug_every == 0:
            ...
            logger.info(
                ...
                f"total={total_reward.mean().item():.4f}, "
                f"progress={progress_reward.mean().item():.4f}, "
                f"forward_vel={forward_vel.mean().item():.4f}, "
